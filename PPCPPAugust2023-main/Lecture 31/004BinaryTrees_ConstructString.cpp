...
string tree2str(TreeNode* root) {
	...
	if (root == NULL) {
		...
		// return string repr of empty tree
	...
	if (root->left != NULL and root->right != NULL) { // leftRepr != "" and rightRepr != ""

		return to_string(root->val) + "(" + leftRepr + ")(" + rightRepr + ")";

	} else if (root->left != NULL) { // leftRepr != ""

		return to_string(root->val) + "(" + leftRepr + ")";

	} else if (root->right != NULL) { // rightRepr != ""

		return to_string(root->val) + "()(" + rightRepr + ")";

	}

	// root->left == NULL and root->right == NULL // leftRepr == "" and rightRepr == ""
