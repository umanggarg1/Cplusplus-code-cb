...
int dfsHelper(vector<vector<int>> grid,
	...
	// if (nx >= 0 and nx < m and ny >= 0 and ny < n and grid[nx][ny] == 1 and visited[nx][ny] == false) {
...
int largestIsland(vector<vector<int>> grid, int m, int n) {
	...
	int maxSoFarArea = 0; // to keep track of the area of the largest island
	for (int i = 0; i < m; i++) {
		for (int j = 0; j < n; j++) {
			if (grid[i][j] == 1 and !visited[i][j]) {
				maxSoFarArea = max(maxSoFarArea,
