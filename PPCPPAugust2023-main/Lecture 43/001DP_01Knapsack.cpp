...
int helper(vector<int> weights, vector<int> profits, int N, int i, int w) {
	...
	// option 1 : include the ith object in the knapsack

	// option 2 : exclude the ith object from the knapsack
...
int helperBottomUp(vector<int> weights, vector<int> profits, int N, int W) {
	...
	for (int i = N - 1; i >= 0; i--) {
		for (int w = 1; w <= W; w++) {
			...
			if (weights[i] > w) {
				// exlude the ith object from the knapsack
				dp[i][w] = dp[i + 1][w];
			} else {
	...
	return dp[0][W]; // at the (0, W)th cell, you store f(0, W)
...
int helperBottomUpSpaceOptimised(vector<int> weights, vector<int> profits, int N, int W) {
	...
	for (int i = N - 1; i >= 0; i--) {
		for (int w = W; w >= 1; w--) {
			...
			if (weights[i] > w) {
				...
				dp[w] = max(profits[i] + dp[w - weights[i]], dp[w]);
			}
	...
	return dp[W]; // at the (0, W)th cell, you store f(0, W)
