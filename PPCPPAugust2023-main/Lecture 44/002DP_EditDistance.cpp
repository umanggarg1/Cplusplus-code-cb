...
int editDistance(string s1, string s2, int m, int n, int i, int j) {
	...
	// option 1 : letter insertion

	// option 2 : letter deletion

	// option 3 : letter substitution

	return min({
		1 + editDistance(s1, s2, m, n, i, j + 1),
		1 + editDistance(s1, s2, m, n, i + 1, j),
		static_cast<int>(s1[i] != s2[j]) + editDistance(s1, s2, m, n, i + 1, j + 1)
	});
	...
	for (int i = m - 1; i >= 0; i--) {
		for (int j = n - 1; j >= 0; j--) {
			...
			dp[i][j] = min({1 + dp[i][j + 1],
			                1 + dp[i + 1][j],
			                static_cast<int>(s1[i] != s2[j]) + dp[i + 1][j + 1]
			               });
