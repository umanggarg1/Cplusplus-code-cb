		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				if (dfsHelper(neighbour, visited, stackState)) {
					// you've found a backedge in the subcomponent
				...
				// check if source->neighbour is backedge

				// source->neighbour is a backedge when there is a directed path from neighbour to source

				if (stackState.find(neighbour) != stackState.end()) {
					// there is directed path from neighbour to source therefore source->neighbour is a backedge
					return true;
				}
