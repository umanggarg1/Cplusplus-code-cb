...
TreeNode* buildTree() {
	...
	if (data == -1) {
		// build and return an empty tree
		return NULL;
	}

	TreeNode* root = new TreeNode(data);

	// ask your friend to build the leftSubtree using its preOrder traversal

	root->left = buildTree();

	// ask your friend to build the rightSubtree using its preOrder traversal
