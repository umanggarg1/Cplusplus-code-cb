...
int helper(vector<int>& nums, int n, int i) {
    ...
    // option 2 : do not rob the ith house
...
int helperTopDown(const vector<int>& nums, int n, int i, vector<int>& dp) {
    ...
    if (dp[i] != -1) {
        // you already know max. money that can be robbed from houses[i...n-1]
        return dp[i];
    }
...
int helperBottomUp(const vector<int>& nums, int n) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        // dp[i] => f(i) : max money that can be robbed from houses[i...n-1]
