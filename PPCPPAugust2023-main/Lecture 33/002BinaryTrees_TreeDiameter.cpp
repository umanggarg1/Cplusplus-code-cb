...
int computeDiameter(TreeNode* root) {
	...
	// 3. compute the length of the longest path between any two nodes that passes through the root node
	int longestPathThroughRoot = computeHeight(root->left) + computeHeight(root->right) + 2;

	return max({leftDiameter, rightDiameter, longestPathThroughRoot});

	// max(a, max(b, c)) or max({a, b, c})
	// max(a, max(b, max(c, d))) or max({a, b, c, d})
...
Pair computeDiameterOptimised(TreeNode* root) {
	...
	// 3. compute the length of the longest path between any two nodes that passes through the root node
	int longestPathThroughRoot = left.height + right.height + 2;

	p.diameter = max({left.diameter, right.diameter, longestPathThroughRoot});
...
int main() {
	...
	cout << computeDiameterOptimised(root).diameter << endl;
