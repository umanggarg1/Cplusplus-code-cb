...
TreeNode* deleteFromBST(TreeNode* root, int key) {
	...
	if (key < root->val) {
		// ask your friend to delete the key, if present, from the LST
		root->left = deleteFromBST(root->left, key);
	} else if (key > root->val) {
		// ask your friend to delete the key, if present, from the RST
		...
		if (root->left == NULL and root->right == NULL) {
			...
			// TreeNode* rightMin = findMinmum(root->right);
